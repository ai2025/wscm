...
class TentangBKK extends Component
{
    ...
    public function create()
    {
        if ($this->readBlog()->count()) {
            session()->flash('msgWar', 'Blog Tentang BKK sudah ada, mohon lakukan update.');
            return redirect()->route('showTbkkPage');
        } else {
    ...
    {
        return Blog::select('*')->where('tag', 'tentang_bkk')->get();
    }
    ...
    {
        return view('livewire.bkk.tentang-b-k-k', [
            'data'=> $this->read(),
            'blog' => $this->readBlog(),
