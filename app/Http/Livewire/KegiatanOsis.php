...
class KegiatanOsis extends Component
{
    ...
    public function create()
    {
        if ($this->readBlog()->count()) {
            session()->flash('msgWar', 'Blog Kegiatan Osis sudah ada, mohon lakukan update.');
            return redirect()->route('showOsisPage');
        } else {
    ...
    {
        return Blog::select('*')->where('tag', 'kegiatan_osis')->get();
    }
    ...
    {
        return view('livewire.kesiswaan.kegiatan-osis', [
            'data'=> $this->read(),
            'blog' => $this->readBlog(),
