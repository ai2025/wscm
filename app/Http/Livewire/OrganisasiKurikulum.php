...
class OrganisasiKurikulum extends Component
{
    ...
    public function create()
    {
        if ($this->readBlog()->count()) {
            session()->flash('msgWar', 'Blog Organisasi Kurikulum sudah ada, mohon lakukan update.');
            return redirect()->route('showSOkurikulumPage');
        } else {
    ...
    {
        return Blog::select('*')->where('tag', 'organisasi_kurikulum')->get();
    }
    ...
    {
        return view('livewire.kurikulum.organisasi-kurikulum', [
            'data'=> $this->read(),
            'blog' => $this->readBlog(),
