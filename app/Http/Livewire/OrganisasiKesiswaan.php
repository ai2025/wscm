...
class OrganisasiKesiswaan extends Component
{
    ...
    public function create()
    {
        if ($this->readBlog()->count()) {
            session()->flash('msgWar', 'Blog Organisasi (Kesiswaan) sudah ada, mohon lakukan update.');
            return redirect()->route('showSOSiswaPage');
        } else {
    ...
    {
        return Blog::select('*')->where('tag', 'organisasi_kesiswaan')->get();
    }
