...
class OrganisasiHumas extends Component
{
    ...
    public function create()
    {
        if ($this->readBlog()->count()) {
            session()->flash('msgWar', 'Blog Organisasi (Humas) sudah ada, mohon lakukan update.');
            return redirect()->route('showSOHumasPage');
        } else {
    ...
    {
        return Blog::select('*')->where('tag', 'organisasi_humas')->get();
    }
