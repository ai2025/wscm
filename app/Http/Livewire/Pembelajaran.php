...
class Pembelajaran extends Component
{
    ...
    public function create()
    {
        if ($this->readBlog()->count()) {
            session()->flash('msgWar', 'Blog Pembelajaran sudah ada, mohon lakukan update.');
            return redirect()->route('showPembelajaranPage');
        } else {
    ...
    {
        return Blog::select('*')->where('tag', 'pembelajaran')->get();
    }
