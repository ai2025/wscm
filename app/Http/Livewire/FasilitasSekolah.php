...
class FasilitasSekolah extends Component
{
    ...
    public function create()
    {
        if ($this->readBlog()->count()) {
            session()->flash('msgWar', 'Blog Fasilitas Sekolah sudah ada, mohon lakukan update.');
            return redirect()->route('showFasilitasPage');
        } else {
    ...
    {
        return Blog::select('*')->where('tag', 'fasilitas')->get();
    }
    ...
    {
        return view('livewire.sarpras.fasilitas-sekolah', [
            'data'=> $this->read(),
            'blog' => $this->readBlog(),
