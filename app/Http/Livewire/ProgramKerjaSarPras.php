...
class ProgramKerjaSarPras extends Component
{
    ...
    public function create()
    {
        if ($this->readBlog()->count()) {
            session()->flash('msgWar', 'Blog Program Kerja (SarPras) sudah ada, mohon lakukan update.');
            return redirect()->route('showPKSarprasPage');
        } else {
    ...
    {
        return Blog::select('*')->where('tag', 'program_kerja_sarpras')->get();
    }
