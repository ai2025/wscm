...
class Akreditasi extends Component
{
    ...
    public function create()
    {
        if ($this->readBlog()->count()) {
            session()->flash('msgWar', 'Blog Akreditasi sudah ada, mohon lakukan update.');
            return redirect()->route('showAkreditasiPage');
        } else {
    ...
    {
        return Blog::select('*')->where('tag', 'akreditasi')->get();
    }
