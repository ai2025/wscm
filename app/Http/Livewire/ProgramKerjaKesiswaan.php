...
class ProgramKerjaKesiswaan extends Component
{
    ...
    public function create()
    {
        if ($this->readBlog()->count()) {
            session()->flash('msgWar', 'Blog Program Kerja Kesiswaan sudah ada, mohon lakukan update.');
            return redirect()->route('showPKSiswaPage');
        } else {
    ...
    {
        return Blog::select('*')->where('tag', 'program_kerja_kesiswaan')->get();
    }
    ...
    {
        return view('livewire.kesiswaan.program-kerja-kesiswaan', [
            'data'=> $this->read(),
            'blog' => $this->readBlog(),
