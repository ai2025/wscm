...
class OrganisasiSarPras extends Component
{
    ...
    public function create()
    {
        if ($this->readBlog()->count()) {
            session()->flash('msgWar', 'Blog Organisasi (SarPras) sudah ada, mohon lakukan update.');
            return redirect()->route('showSOSarprasPage');
        } else {
    ...
    {
        return Blog::select('*')->where('tag', 'organisasi_sapras')->get();
    }
