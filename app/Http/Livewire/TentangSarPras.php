...
class TentangSarPras extends Component
{
    ...
    public function create()
    {
        if ($this->readBlog()->count()) {
            session()->flash('msgWar', 'Blog Tentang (SarPras) sudah ada, mohon lakukan update.');
            return redirect()->route('showTSarprasPage');
        } else {
    ...
    {
        return Blog::select('*')->where('tag', 'tentang_sarpras')->get();
    }
    ...
    {
        return view('livewire.sarpras.tentang-sar-pras', [
            'data'=> $this->read(),
            'blog' => $this->readBlog(),
