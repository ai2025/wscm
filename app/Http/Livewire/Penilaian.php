...
class Penilaian extends Component
{
    ...
    public function create()
    {
        if ($this->readBlog()->count()) {
            session()->flash('msgWar', 'Blog Penilaian sudah ada, mohon lakukan update.');
            return redirect()->route('showPenilaianPage');
        } else {
    ...
    {
        return Blog::select('*')->where('tag', 'penilaian')->get();
    }
    ...
    {
        return view('livewire.kurikulum.penilaian', [
            'data'=> $this->read(),
            'blog' => $this->readBlog(),
