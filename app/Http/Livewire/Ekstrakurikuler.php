...
class Ekstrakurikuler extends Component
{
    ...
    public function create()
    {
        if ($this->readBlog()->count()) {
            session()->flash('msgWar', 'Blog EKstrakurikuler sudah ada, mohon lakukan update.');
            return redirect()->route('showEkskulPage');
        } else {
    ...
    {
        return Blog::select('*')->where('tag', 'ekskul')->get();
    }
    ...
    {
        return view('livewire.kesiswaan.ekstrakurikuler', [
            'data'=> $this->read(),
            'blog' => $this->readBlog(),
